  ...
  def self.filter(controller)
    ...
    # out, so the existence of the cookie always has to be checked.
      ...
      if request.protocol !~ %r{^https}
        ...
        if @@debug_enabled
          umnauthlog controller, "Cookie #{@@cookiename} not present from #{request.remote_ip}."
        else
...
#        end
        intended_destination = "#{request.protocol}#{request.server_name}#{request.request_uri}"
          
      controller.send(:session)[:umnauth] = nil
      #controller.send :redirect_to, "#{@@authentication_redirect}#{intended_destination}"
###NEW
  ...
  def self.umnauthlog( controller, str, level=:info )
    controller.send( "logger" ).method(level).call("[#{@@filter_name}] #{str}") if @@logging_enabled
  end
  ...
  def self.check_cookie_with_authority( controller, str )
    ...
    http.use_ssl = true
    path = "/WEBCOOKIE?x&#{CGI.escape(str)}"
    get = Net::HTTP::Get.new( path )
    umnauthlog controller, "Sending HTTP GET request\nSSL?: #{http.use_ssl?}\nServer: #{@@x500_server}\nPort: #{@@x500_https_port}\nPath: #{path}"
    retval = http.request( get ).body.strip
